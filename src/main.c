...
void neuralNetTask(void *params) {
    ...
    for (;;) {
        ...
        if (g_mode == MODE_TRAINING) {
            ...
            const uint32_t EARLY_TO_CURRENT_JOY_TOLERANCE = 256 * 256;
            ...
            while(!triggered){
                ...
                dx = (early_joy.x - joy.x);
                dy = (early_joy.y - joy.y);
                
                if ( (dx * dx + dy * dy) == EARLY_TO_CURRENT_JOY_TOLERANCE * EARLY_TO_CURRENT_JOY_TOLERANCE ){
                    triggered = true; 
                } else{
                    early_joy.x = joy.x;
                    early_joy.y = joy.y;
