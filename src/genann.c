...
double genann_act_relu(const genann *ann unused, double a) {
    return a > 0 ? a : 0;
}
...
void genann_train(genann const *ann, double const *inputs, double const *desired_outputs, double learning_rate) {
    ...
    for (h = ann->hidden_layers - 1; h >= 0; --h) {
        ...
        for (j = 0; j < ann->hidden; ++j) {
            ...
            /* Derivada da função de ativação oculta aplicada à saída *o:
             *   sigmoid : f'(o) = o * (1 - o)       (derivada em termos da saída)
             *   ReLU    : f'(o) = 1 se o > 0, 0 cc  (threshold na saída)
             * Qualquer outra função usa a aproximação sigmoid por padrão. */
            double act_grad;
